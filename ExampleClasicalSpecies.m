...
%% Run example
% Here we provide an example with a clasical molecule.
% The obtained result is also compared to the reference species.
% As such it is expected that the results are identical.
...
% the new species contains all electronic states included in MolFile
Elecs = 'All';
...
ax = subplot(3,1,3);
hold on;
plot(T, CpT( Sp.pol, T, Sp.Tswitch) - CpT( Spref.pol, T, Spref.Tswitch));

xlim([250 6000]);
xlabel('$T$ [K]','Interpreter','latex');
ylabel('$C_p - C_{p,\mathrm{ref}}$ [J/mol/K]','Interpreter','latex')
grid on
